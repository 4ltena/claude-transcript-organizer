...
transcript_organizer/
  ...
  pipeline.py                organize / status / render を束ねる中核
...
| `route.py` | `route(cwd, config) -> Target` / `label_to_root(label, config) -> str` |
...
| `pipeline.py` | `organize(config, provider, ...) -> dict` / `status(config) -> dict` / `render(config, only_label, dry_run) -> dict` / `current_protect(config) -> set` |
...
`pipeline.render` は LLM を使わず、保存済み findings から HANDOFF を描き直す。中断やプロセス kill で organize の最終描画段に届かず HANDOFF が未更新のまま残った場合の復旧用。`data/findings/*.json` を走査し、各 label について `store.load` が空なら `empty`、`route.label_to_root` で逆算した root のディレクトリが無ければ `missing_root`（削除済みプロジェクトを作り直さない）でスキップし、それ以外を `render_markdown` → `update_handoff` で更新する。`label_to_root` は label を `__` で分割して `PROJECTS` root と結合する route の逆写像のため、organize と同じ設定で実行する必要がある。`organize --rebuild`（台帳無視の LLM 再抽出）とは別物で、課金を伴わない。
