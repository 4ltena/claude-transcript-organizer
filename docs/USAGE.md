...
| `render` | 保存済み findings から HANDOFF を再描画（LLM 不使用） | 低（HANDOFF の管理区間のみ書く） |

推奨フローは、`status` で様子を見て、`organize` で抽出し、HANDOFF の中身を確認してから、`delete` で候補を確認し、`delete --yes` で退避する。`organize` は終了時に削除するか確認するので、HANDOFF を確認しなくてよいなら `organize -y` で抽出から退避まで一気に進められる。
...
| | `--yes` / `-y` | 整理後の削除確認を省き、自動で trash 退避まで実行 |
...
| `render` | `--config <path>` | 設定ファイルを差し替える |
| | `--project <label>` | 指定ラベルだけ再描画 |
| | `--dry-run` | 対象件数だけ表示し、書き込まない |
...
# 整理後そのまま削除（trash 退避）まで実行（確認プロンプトを省く）
python cli.py organize -y
```

整理が終わると、続けて削除（trash 退避）するか確認する。`y` で `delete --yes` 相当を実行し、それ以外なら何もしない。`-y`（`--yes`）を付けると確認を省いて自動で退避まで進む。`--dry-run` 時は確認しない。非対話（パイプ等）で入力が無い場合は「削除しない」に倒す。
...
trash へ退避した会話は、台帳（`data/ledger.json`）からも同時に取り除く。退避後のファイルは走査対象から外れて再発見されないので、実体のない孤児エントリが台帳に残らず、`status` の台帳件数も実状と合う。`organize -y` 経由の退避でも同じ。

---

## 5. `render` — findings から HANDOFF を作り直す

中断やプロセス kill で「findings は保存済みだが HANDOFF が未更新／古い」まま残ったとき、保存済み findings から HANDOFF を **LLM なしで**再生成する。`organize --rebuild` は台帳を無視して LLM 抽出からやり直す（課金あり）のに対し、`render` は抽出済みの findings をそのまま描き直すだけなので速くて無料。
...
# 特定ラベルだけ
python cli.py render --project Webs__portfolio

# 対象を確認するだけ（書き込まない）
python cli.py render --dry-run
```

```
再描画: 22件 / HANDOFF: 22件 / スキップ: {'empty': 12}
```

出力先 root はラベルから逆算する（`route` の逆。`Webs__portfolio` → `<PROJECTS>/Webs/portfolio`、`_archive` → `archive_root`）。そのため `organize` と同じ設定（`PROJECTS` ルート）で実行する必要がある。findings が空のラベルは `empty`、逆算した root のディレクトリが存在しないラベルは `missing_root` でスキップし、削除済みプロジェクトを作り直さない。

---

## 6. 短縮コマンド（tsorg / tstat / tsdel / tsren）
...
引数はそのまま転送される。末尾が後勝ちなので、その回だけ別プロバイダへ上書きもできる。LLM を使わない `tstat`/`tsdel`/`tsren` は常に Windows 側で実行する。

```
tsorg --dry-run
tsorg -y                  # 整理後そのまま削除まで実行
tsorg --provider anthropic
tsorg --project Webs__portfolio
tsdel --yes
tsren                     # findings から HANDOFF を再描画
```
...
- HANDOFF を作り直したい → 抽出済みなら `render`（LLM 不使用・無料）で十分。findings ごと取り直したいときだけ `organize --rebuild`（LLM 再抽出）。
